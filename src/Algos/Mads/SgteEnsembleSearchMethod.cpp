...
#include "../../Algos/Mads/SgteEnsembleSearchMethod.hpp"
#ifdef USE_SGTELIB
...
void NOMAD::SgteEnsembleSearchMethod::init()
{
    setName("Sgte Ensemble Search Method");
    //setComment("(SgteEnsembleAlgo)");
    verifyParentNotNull();
    ...
    if (isEnabled())
    {
        ...
        // Create the SgteEnsembleAlgo algorithm with its own stop reasons
        ...
        _modelAlgo = std::make_shared<NOMAD::SgteEnsembleAlgo>(this, stopReasons,
                                                           barrier, _runParams,
                                                           _pbParams, mesh);
...
bool NOMAD::SgteEnsembleSearchMethod::runImp()
{
    // SgteEnsembleAlgo algorithm _modelAlgo was created in init().
...
void NOMAD::SgteEnsembleSearchMethod::generateTrialPointsImp()
{
#ifdef USE_SGTELIB
    std::string s;
    ...
    // SgteEnsembleSearchMethod is processed on all points of the barrier.
    ...
    else if (!_stopReasons->checkTerminate())
    {
        ...
        if (0 == oraclePoints.size())
        {
            ...
            {
                throw NOMAD::Exception(__FILE__, __LINE__, "SgteEnsembleAlgo Algorithm must have a Sgtelib stop reason");
            }
...
void NOMAD::SgteEnsembleSearchMethod::getBestProjection(const NOMAD::Point& incumbent,
