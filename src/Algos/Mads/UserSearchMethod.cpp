...
void NOMAD::UserSearchMethod::init()
{
    ...
    {
        throw NOMAD::Exception(__FILE__,__LINE__,"UserSearchMethod must have an iteration ancestor");
    }
...
void NOMAD::UserSearchMethod::generateTrialPointsFinal()
{
    ...
    if (nullptr == barrier)
    {
        throw NOMAD::Exception(__FILE__,__LINE__,"UserSearchMethod must have a MadsMegaIteration ancestor with a barrier");
    }
    ...
    auto mads = dynamic_cast<const NOMAD::Mads*>(_iterAncestor->getRootAlgorithm());
    bool success =false;
    EvalPointSet userSearchPoints;
    auto cb = mads->getCallback<NOMAD::MadsCallbackType::USER_METHOD_SEARCH>();
    if (cb)
        success = cb->call(*this, userSearchPoints);
    
    if (!success)
    {
        OUTPUT_INFO_START
        AddOutputInfo("User search cannot produce trial points.");
        OUTPUT_INFO_END
        return;
    }
    
    // Insert the point. Projection on mesh and snap to bounds is done later
    for (const auto & tp : userSearchPoints)
    {
        NOMAD::EvalPoint evalPoint(tp);
...
{
    auto mads = dynamic_cast<const NOMAD::Mads*>(_iterAncestor->getRootAlgorithm());
    auto cb = mads->getCallback<NOMAD::MadsCallbackType::USER_METHOD_SEARCH_END>();
    if (cb)
    {
        ...
        {
            OUTPUT_INFO_START
            AddOutputInfo("User search post evaluation is not working properly.");
            OUTPUT_INFO_END
            return;
        }
    }
