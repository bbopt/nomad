...
#include "../../Algos/SgteEnsembleRenaud/SgteEnsembleEvaluator.hpp"
#include "../../Algos/SgteEnsembleRenaud/SgteEnsembleInitialization.hpp"
#include "../../Algos/SgteEnsembleRenaud/SgteEnsembleMegaIteration.hpp"
...
void NOMAD::SgteEnsembleAlgo::init()
{
    setName("SgteEnsembleAlgo");
    verifyParentNotNull();
    ...
    auto modelDefinition = _runParams->getAttributeValue<NOMAD::ArrayOfString>("SGTELIB_MODEL_DEFINITION"); // todo : must always be ENSEMBLE (renaud)
    ...
    const auto bbot = NOMAD::SgteEnsembleAlgo::getBBOutputType();
    ...
    _initialization = std::make_unique<NOMAD::SgteEnsembleInitialization>(this);
}
...
/*       Destructor        */
/*-------------------------*/
NOMAD::SgteEnsembleAlgo::~SgteEnsembleAlgo()
{
    reset();
}
...
/*           reset         */
/*-------------------------*/
void NOMAD::SgteEnsembleAlgo::reset()
{
    if (nullptr != _model)
    {
        _model.reset();
...
/*-------------------------------------------------------------*/
/*                      isReady                                */
/* Return a boolean to know if the SgteEnsembleAlgo model has  */
/* been built and can be called for predictions.               */
/*-------------------------------------------------------------*/
bool NOMAD::SgteEnsembleAlgo::isReady() const
{
    ...
    if (!retReady)
    {
        if (!_trainingSet)
        {
            throw NOMAD::Exception ( __FILE__, __LINE__ ,
                                    "SgteEnsembleAlgo::isReady : no training set");
        }
...
/*           info          */
/*-------------------------*/
void NOMAD::SgteEnsembleAlgo::info()
{
    std::cout << "  #===================================================== #" << std::endl;
    std::cout << "SgteEnsembleAlgo::info" << std::endl ;
    std::cout << "SgteEnsembleAlgo : " << this << std::endl;
    ...
    if ( _ready )
    {
        std::cout << "SgteEnsembleAlgo model is ready" << std::endl;
    }
    else
    {
        std::cout << "SgteEnsembleAlgo model is NOT ready" << std::endl;
    }
...
// Used during SgteEnsembleUpdate step.
// Update the bounds of the model.
void NOMAD::SgteEnsembleAlgo::setModelBounds(std::shared_ptr<SGTELIB::Matrix> X)
{
    size_t n = _pbParams->getAttributeValue<size_t>("DIMENSION");
    if (n != (size_t)X->get_nb_cols())
    {
        throw NOMAD::Exception(__FILE__, __LINE__,
                               "SgteEnsembleAlgo::setModelBounds() dimensions do not match");
    }
...
/*                          Extended Bounds                               */
/*------------------------------------------------------------------------*/
NOMAD::ArrayOfDouble NOMAD::SgteEnsembleAlgo::getExtendedLowerBound() const
{
...
NOMAD::ArrayOfDouble NOMAD::SgteEnsembleAlgo::getExtendedUpperBound() const
{
...
// Start is executed when SgteEnsembleAlgo is used as an algorithm on its own (never happens (renaud)).
void NOMAD::SgteEnsembleAlgo::startImp()
{
...
bool NOMAD::SgteEnsembleAlgo::runImp()
{
    ...
    if (!_termination->terminate(k))
    {
        ...
        while (!_termination->terminate(k))
        {
            ...
            NOMAD::SgteEnsembleMegaIteration megaIteration(this, k, barrier, megaIterSuccess);
            megaIteration.start();
            megaIteration.run();
            megaIteration.end();
        ...
        // Update it here.
        _megaIteration = std::make_shared<NOMAD::SgteEnsembleMegaIteration>(this, k, barrier, megaIterSuccess);
    }
...
void NOMAD::SgteEnsembleAlgo::endImp()
{
...
/*------------------------------------------------------------------------*/
const NOMAD::SgtelibModelFormulationType NOMAD::SgteEnsembleAlgo::getFormulation() const
{
...
NOMAD::Double NOMAD::SgteEnsembleAlgo::getDeltaMNorm() const
{
...
/*------------------------------------------------------------------------*/
void NOMAD::SgteEnsembleAlgo::checkHF(NOMAD::EvalPoint& x) const
{
    ...
    if (!f.isDefined())
    {
        ...
        AddOutputWarning("Warning: SgteEnsembleAlgo::checkHF(): f is not defined and needs to be recomputed.");
    }
    ...
    if (!h.isDefined())
    {
        ...
        const auto bbot = NOMAD::SgteEnsembleAlgo::getBBOutputType();
        NOMAD::SgteEnsembleEvaluator::evalH(bbo, bbot, h);
        AddOutputWarning("Warning: SgteEnsembleAlgo::checkHF(): h is not defined and needs to be recomputed.");
    }
...
/*------------------------------------------------------------------------*/
NOMAD::Double NOMAD::SgteEnsembleAlgo::getFMin() const
{
    NOMAD::Double fMin;
...
size_t NOMAD::SgteEnsembleAlgo::getNbModels(const NOMAD::SgtelibModelFeasibilityType modelFeasibility,
...
std::vector<NOMAD::EvalPoint> NOMAD::SgteEnsembleAlgo::getX0s() const
{
...
// To be used outside of SgteEnsembleAlgo, e.g., in SgteEnsembleSearchMethod.
NOMAD::EvalPointSet NOMAD::SgteEnsembleAlgo::createOraclePoints()
{
    ...
    NOMAD::SgteEnsembleMegaIteration megaIteration(this, 0, _barrierForX0s, NOMAD::SuccessType::NOT_EVALUATED);
