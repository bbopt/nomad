                                                           ...
                                                           const double mu)
{
                  ...
                  const SGTELIB::Matrix& ub)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            if (li != -NOMAD::INF)
            {
                if (di < 0.0)
                {
                    const double tmp_step_size = (xi - li) / std::fabs(di);
            ...
            if (ui != NOMAD::INF)
            {
                if (di > 0.0)
                {
                    const double tmp_step_size = (ui - xi) / std::fabs(di);
                    max_step_size = std::min(max_step_size, tmp_step_size);
                }
            }
        }
    ...
    while (std::fabs(alpha - b) > min_threshold)
    {
