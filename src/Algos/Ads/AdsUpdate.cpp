...
void NOMAD::AdsUpdate::init()
{
    ...
    if (nullptr == megaIter)
    {
        throw NOMAD::Exception(__FILE__,__LINE__,"Error: An instance of class AdsUpdate must have a MegaIteration among its ancestors");
    }
...
bool NOMAD::AdsUpdate::runImp()
{
    ...
    {
        // Compute success
        // Get which of newBestFeas and newBestInf changed
        ...
        {
            // newBestFeas is the point that changed.
            newBest = newBestFeas;
        ...
        else
        {
            ...
            if (success >= NOMAD::SuccessType::PARTIAL_SUCCESS)
            {
                // newBestInf is the point that changed.
                newBest = newBestInf;
                // Workaround: If we do not have the point from which newBest was computed,
        ...
        // ADS-specific: if the iteration is currently PARTIAL_SUCCESS,
        // validate this iteration status against the punctured-space rule.
        if (success == NOMAD::SuccessType::PARTIAL_SUCCESS && nullptr != newBest)
        {
            ...
            {
                // Get iteration tag
                size_t iterationTag = NOMAD::EvalPoint::getCurrentTag();
                
                // Calculate minDelta from mesh
                auto deltaMeshSize = mesh->getdeltaMeshSize();
                ...
                // Check if the new best point is in the punctured space.
                bool isInPuncturedSpace = adsBarrier->isInPuncturedSpace(*newBest->getX(), minDelta, iterationTag);
                
                if (isInPuncturedSpace)
                {
                    // Status remains PARTIAL_SUCCESS; register the point tag in ADS memory.
                    adsBarrier->addCachePointTag(newBest->getTag());
                    
                    OUTPUT_INFO_START
                    AddOutputInfo("Update: iteration PARTIAL_SUCCESS : the point is in punctured space. Tag added to barrier.");
                    OUTPUT_INFO_END
                }
                else
                {
                    // The point is outside the punctured space. Change the iteration status to UNSUCCESSFUL.
                    success = NOMAD::SuccessType::UNSUCCESSFUL;
                    
                    OUTPUT_INFO_START
                    AddOutputInfo("Update: iteration PARTIAL_SUCCESS : the point is outside punctured space. Changed to UNSUCCESSFUL.");
                    OUTPUT_INFO_END
                }
            }
        }
        ...
        {   // The iteration is considered as Reframing if the search step produced a dominating point that is not in punctured space and then the poll failed. It is considered as unsuccessful if the search failed to produce a dominating point, the poll failed to produce a dominating point and the update step failed to find an improving point.
            OUTPUT_INFO_START
            AddOutputInfo("Last Iteration Unsuccessful or Reframing. Delta is refined.");
            OUTPUT_INFO_END
    ...
    // CatAds initial DOE is performed during algo initialization (MadsInitialization).
