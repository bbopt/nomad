...
% % Blackbox Function
% %clc
% bb = @(x) [29.4*x(1) + 18*x(2),-(x(1) - 0.2458*x(1)^2/x(2)) + 6];
% % Bounds      
% lb = [0;1e-5];
% ub = [115.8;30];
% % Starting Guess
...
% PROBLEM 4 [fval = -2.5]
% %clc
% fun = @(x)   [-x(1) - x(2) - x(3),(x(2) - 1./2.)*(x(2) - 1./2.) + (x(3) - 1./2.)*(x(3) - 1./2.) - 1/4, x(1) - x(2),x(1) + x(3) + x(4) - 2];
% ub = [1;10;10;5];
% lb = [0;0;0;0];
% x0 = [0;0;0;0];
% 
...
% %clc
% % Blackbox Function
% bb = @(x) [2*x(1)+x(2),-16*x(1)*x(2) + 1,(-4*x(1)^2) - 4*x(2)^2 + 1];
% % Bounds       
% lb = [0;0];
% ub = [1;1];
% % Starting Guess
...
% %clc
% % Fitting Function
% obj = @(x) [x(1) - sin(2*x(1) + 3*x(2)) - cos(3*x(1) - 5*x(2)), x(2) - sin(x(1) - 2*x(2)) + cos(x(1) + 3*x(2))];
% % Blackbox Function      
% bb = @(x) norm(obj(x));
% 
% % Bounds
...
% %% MINLP 1 [fval = -5]
% clc
% fun = @(x) [ (x(1) - 5)^2 + x(2)^2 - 25,x(1)^2 - x(2) + 0.5 ];
% x0 = [0;0];
