...
Alternative Mads algorithms: ADS and MadsPIP
---------------------------------------------

NOMAD provides two variants of the Mads algorithm that can be selected instead of the default Mads. These algorithm-selection parameters (ending with the word "OPTIMIZATION") are mutually exclusive: only one of them (or none, to use the default Mads) can be set to ``true`` at a time.

``ADS_OPTIMIZATION``
"""""""""""""""""""""

Setting ``ADS_OPTIMIZATION`` to ``true`` enables the Adaptive Direct Search (ADS) algorithm, a variant of Mads that does not project trial points onto the mesh [AuDeDiLeDTr2026]_. This is especially useful when trial points are generated by a custom search or poll method (see :ref:`custom_search_and_poll_methods`) and should not be snapped back onto the mesh. An example of ADS combined with a custom search method is provided in ``$NOMAD_HOME/examples/advanced/library/CustomSearchMethod``.
...
.. _custom_search_and_poll_methods:

Custom search and poll methods
------------------------------

In library mode, the trial points generated during the *Search* and *Poll* steps of Mads can be customized by registering a user callback function. This lets the user try search or poll strategies of their own in addition to (or, for the poll, combined with) the ones already available in NOMAD.

To add a custom search method, set the parameter ``USER_SEARCH`` to ``true`` and register a callback function of type ``NOMAD::MadsCallbackType::USER_METHOD_SEARCH``. The callback receives the current step and must fill in the set of trial points to evaluate.

An example is provided in ``$NOMAD_HOME/examples/advanced/library/CustomSearchMethod``. The callback function ``userSearchCallbackFn`` generates a single trial point along a random direction on the unit sphere, scaled beyond the current mesh frame size, and inserts it into the trial point set:

To add a custom poll method, add ``USER_POLL`` to the parameter ``DIRECTION_TYPE`` (it can be combined with another direction type, such as ``ORTHO_2N``) and register a callback function of type ``NOMAD::MadsCallbackType::USER_METHOD_POLL``. The callback receives the current step and the problem dimension, and must fill in a list of poll directions.

An example is provided in ``$NOMAD_HOME/examples/advanced/library/CustomPollMethod``. The callback function ``myQRPollCallback`` builds a set of 2N poll directions by a QR factorization of a matrix whose first column is a random unit direction:

Note that by default, user search and poll callbacks are not called during the quadratic model search.
...
In library mode, NOMAD lets the user register callback functions on ``NOMAD::MainStep`` to monitor or customize the behavior of an optimization run, using ``addCallback<CallbackType>(function)``. Only one callback can be registered for a given type. In addition to the ``NOMAD::MadsCallbackType`` callbacks used for custom search and poll methods (see :ref:`custom_search_and_poll_methods`), NOMAD provides ``NOMAD::EvalCallbackType`` callbacks, triggered around blackbox evaluations, and ``NOMAD::AlgoCallbackType`` callbacks, triggered around algorithmic iterations.

Custom opportunistic strategy
"""""""""""""""""""""""""""""

By default, the opportunistic strategy (see :ref:`eval_opportunistic`) stops the evaluations of a list of trial points as soon as an improved feasible value is found. This behavior can be replaced by a user-defined criterion by registering a callback of type ``NOMAD::EvalCallbackType::EVAL_OPPORTUNISTIC_CHECK``, called after each evaluation. The callback can decide to stop the evaluations of the current queue (``opportunisticEvalStop``) and/or the current iteration (``opportunisticIterStop``).

An example is provided in ``$NOMAD_HOME/examples/advanced/library/CustomOpportunistic``. The callback function ``customEvalCallback`` stops the evaluation queue of a *Poll* step as soon as a feasible point improves the current best feasible objective value by more than 10%.

Stop on blackbox evaluation failure
"""""""""""""""""""""""""""""""""""

The optimization can be stopped based on a user-defined condition checked after every evaluation, by registering a callback of type ``NOMAD::EvalCallbackType::EVAL_STOP_CHECK``. The callback receives the evaluated point and sets a global stop flag when the condition is met.

An example is provided in ``$NOMAD_HOME/examples/advanced/library/StopIfBBFails``. The callback function ``customEvalStopCB`` counts the blackbox evaluations that do not return ``EVAL_OK`` and stops the optimization once a maximum number of such failures is reached.

Stop on consecutive failed iterations
"""""""""""""""""""""""""""""""""""""

The optimization can also be stopped based on a condition checked at the end of each (Mega)Iteration, by registering a callback of type ``NOMAD::AlgoCallbackType::MEGA_ITERATION_END``.

An example is provided in ``$NOMAD_HOME/examples/advanced/library/StopOnConsecutiveFails``. The callback function ``userIterationCallback`` retrieves the Mads success statistics of the current MegaIteration and stops the optimization once a maximum number of consecutive failed iterations is reached.
...
By default, NOMAD expects a blackbox executable to display its evaluation outputs on the **standard output**, which NOMAD redirects and parses itself (parameter ``BB_REDIRECTION`` set to ``true`` by default).
In some cases it may be more convenient to let the blackbox program manage its own output file, for example when the blackbox is a wrapper around a third-party simulation tool that already writes its results to a file.

Setting ``BB_REDIRECTION`` to ``no`` (or ``false``) tells NOMAD that the blackbox executable takes care of writing the evaluation outputs itself, instead of displaying them on the standard output.
In that case, the blackbox program must write the outputs (in the order given by ``BB_OUTPUT_TYPE``) into a file named after the input file it received, with the suffix ``.output`` appended (for example, if the input file is ``x_1.txt``, the blackbox must create ``x_1.txt.output``).
Both the input and output file names are fixed for the whole optimization.

An example is provided in ``$NOMAD_HOME/examples/advanced/batch/BBOutputRedirection``. The blackbox executable ``bb_redirect.exe`` reads two variables from the input file, evaluates the objective function and writes the result to ``inputfilename.output``:
...
.. topic:: References
  ...
  .. [AuDeDiLeDTr2026] C. Audet and T. Denorme and Y. Diouane and S. Le Digabel and C. Tribes.
    Adaptive direct search algorithms with relaxable and quantifiable constraints.
    *Cahier du GERAD*, G-2026-39, 2026. https://www.gerad.ca/papers/G-2026-39.pdf
